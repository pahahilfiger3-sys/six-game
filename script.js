...
function selectGiftMode(type) {
    selectedGiftType = type;
    closeGiftModal();
    
    // Показываем тост или алерт
    if(window.Telegram && Telegram.WebApp && Telegram.WebApp.showAlert) {
        Telegram.WebApp.showAlert(`Gift Selected: ${type.toUpperCase()}\nNow tap a player to send!`);
    } else {
        alert(`Gift Selected: ${type.toUpperCase()}\nNow tap a player to send!`);
    }
}

// --- ОТВЕТЫ ИГРОКОВ ---
function showPlayerAnswer(text, isAudio = false) {
    const modal = document.getElementById('answer-modal');
    const textView = document.getElementById('text-view');
    const audioView = document.getElementById('audio-view');
    const textContainer = document.getElementById('player-answer-text');
...
// Эффект печатания (чтобы длинный текст переносился)
function typeWriterEffect(text, element) {
    ...
    function type() {
        if (i < text.length) {
            ...
            i++;
            setTimeout(type, 20); // Скорость печати
        }
    }
    type();
}